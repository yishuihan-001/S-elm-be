    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        if (!(entities instanceof Array) || !entities.length) {
          throw new Error('entities参数错误1')
        } else if (!(entities[0] instanceof Array) || !entities[0].length) {
          throw new Error('entities参数错误2')
      ...
      try {
        ...
        return res.send(Res.Fail(err.message || '添加购物车失败'))
      }
      ...
      try {
        ...
        res.send(Res.Success(cart))
      } catch (err) {
        console.log('保存购物车数据失败')
        res.send(Res.Fail(err.message || '加入购物车失败'))
      }
    })
  }
}
