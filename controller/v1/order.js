...
class Order extends BaseComponent {
  ...
  async postOrder (req, res, next) {
    ...
    form.parse(req, async (err, fields, files) => {
      if (err) {
        ...
        return res.send(Res.Fail(err.message || '下单失败'))
      }
      ...
      try {
        ...
        return res.send(Res.Fail(err.message || '获取订单失败'))
      }
      ...
      try {
        ...
        res.send(Res.Success({ message: '下单成功，请及时付款', need_validation: false }))
      } catch (err) {
        console.log('保存订单数据失败')
        res.send(Res.Fail(err.message || '保存订单失败'))
      }
    })
  }
  async getOrders (req, res, next) {
    ...
    try {
      ...
      res.send(Res.Success(orders))
    } catch (err) {
      console.log('获取订单列表失败', err)
      res.send(Res.Fail(err.message || '获取订单列表失败'))
    }
  }
  async getDetail (req, res, next) {
    ...
    try {
      ...
      res.send(Res.Fail(err.message || '获取订单信息失败'))
    }
  }
  async getAllOrders (req, res, next) {
    ...
    try {
      ...
      res.send(Res.Fail(err.message || '获取订单数据失败'))
    }
  }
  async getOrdersCount (req, res, next) {
    ...
    try {
      ...
      res.send(Res.Fail(err.message || '获取订单数量失败'))
    }
  }
}
