...
class Shop extends AddressComponent {
  ...
  async addShop (req, res, next) {
    ...
    try {
      ...
      return res.send(Res.Fail('获取数据失败'))
    }
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      const newShop = {
        ...
        phone: fields.phone,
        promotion_info:
          fields.promotion_info || '欢迎光临，用餐高峰请提前下单，谢谢',
        ...
        license: {
          ...
          catering_service_license_image:
            fields.catering_service_license_image || ''
        },
        identification: {
          company_name: '',
          identificate_agency: '',
      ...
      try {
        ...
        res.send(Res.Success(newShop))
      } catch (err) {
        console.log('商铺写入数据库失败')
        res.send(Res.Fail('添加商铺失败'))
      }
    })
  }
  // 获取餐馆列表
  async getRestaurants (req, res, next) {
    ...
    try {
      res.send(Res.Success(restaurants))
    } catch (err) {
      res.send(Res.Fail('获取店铺列表数据失败'))
    }
  }
  // 搜索餐馆
  ...
  async getRestaurantDetail (req, res, next) {
    ...
    try {
      ...
      res.send(Res.Success(restaurant))
    } catch (err) {
      console.log('获取餐馆详情失败', err)
      res.send(Res.Fail('获取餐馆详情失败'))
    }
  }
  ...
  async updateshop (req, res, next) {
    ...
    form.parse(req, async (err, fields, files) => {
      if (err) {
        ...
        return res.send(Res.Fail('表单信息错误'))
      }
      const {
        name,
        address,
      ...
      if (+id === 1) {
        return res.send(Res.Fail('此店铺用做展示，请不要修改'))
      }
      try {
        if (!name) {
        ...
        res.send(Res.Success('修改商铺信息成功'))
      } catch (err) {
        console.log(err.message, err)
        res.send(Res.Fail(err.message || '更新商铺信息失败'))
      }
    })
  }
  async deleteResturant (req, res, next) {
    ...
    if (!restaurant_id || !Number(restaurant_id)) {
      ...
      res.send(Res.Fail('restaurant_id参数错误'))
      return
    }
    if (+restaurant_id === 1) {
      return res.send(Res.Fail('此店铺用做展示，请不要删除'))
    }
    try {
      await ShopModel.remove({ id: restaurant_id })
      res.send(Res.Success('删除餐馆成功'))
    } catch (err) {
      console.log('删除餐馆失败', err)
      res.send(Res.Fail('删除餐馆失败'))
    }
  }
}
