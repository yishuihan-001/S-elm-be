...
class Food extends BaseComponent {
  ...
  async initData (restaurant_id) {
    for (let i = 0; i < this.defaultData.length; i++) {
      ...
      const Category = Object.assign({}, defaultData, { id: category_id }, restaurant_id)
  ...
  async getCategory (req, res, next) {
    ...
    try {
      ...
      res.send(Res.Success(category_list))
    } catch (err) {
      console.log('获取餐馆食品种类失败')
      res.send(Res.Fail('获取数据失败'))
    }
  }
  async addCategory (req, res, next) {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        await newFood.save()
        res.send(Res.Success('添加食品种类成功'))
      } catch (err) {
        console.log('保存数据失败')
        res.send(Res.Fail(err.message || '保存数据失败'))
      }
    })
  }
  async addFood (req, res, next) {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        ...
        res.send(Res.Fail(err.message || '添加食品失败'))
        return
      }
      let item_id
      try {
        ...
        return res.send(Res.Fail(err.message || '添加食品失败'))
      }
      ...
      try {
        const [specfoods, specifications] = await this.getSpecfoods(
          fields,
          item_id
        )
  ...
  async getSpecfoods (fields, item_id) {
    ...
    if (fields.specfoods.length < 2) {
      ...
      specfoods.push({
        ...
        specfoods: [],
        specs_name: fields.specfoods[0].specfoods,
        name: fields.name,
        item_id,
        sku_id,
        food_id,
      ...
      for (let i = 0; i < fields.specfoods.length; i++) {
        ...
        specfoods.push({
          ...
          specfoods: [
            {
              name: '规格',
              value: fields.specfoods[i].specfoods
            }
          ],
          specs_name: fields.specfoods[i].specfoods,
          name: fields.name,
          item_id,
          sku_id,
          food_id,
        ...
        specifications[0].values.push(fields.specfoods[i].specfoods)
      }
    }
  ...
  async getMenu (req, res, next) {
    ...
    if (!restaurant_id || !Number(restaurant_id)) {
      ...
      return res.send(Res.Fail('餐馆ID参数错误'))
    }
    let filter
    if (allMenu) {
    ...
    try {
      ...
      res.send(Res.Success(menu))
    } catch (err) {
      console.log('获取食品数据失败', err)
      res.send(Res.Fail(err.message || '获取食品数据失败'))
    }
  }
  async getMenuDetail (req, res, next) {
    ...
    if (!category_id || !Number(category_id)) {
      ...
      return res.send(Res.Fail('Menu ID参数错误'))
    }
    try {
      ...
      res.send(Res.Fail(err.message || '获取Menu详情失败'))
    }
  }
  async getFoods (req, res, next) {
    ...
    try {
      ...
      const foods = await FoodModel.find(filter, '-_id')
        .sort({ item_id: -1 })
        .limit(Number(limit))
        .skip(Number(offset))
      res.send(Res.Success(foods))
    } catch (err) {
      console.log('获取食品数据失败', err)
      res.send(Res.Fail(err.message || '获取食品数据失败'))
    }
  ...
  async updateFood (req, res, next) {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        ...
        res.send(Res.Success('修改食品信息成功'))
      } catch (err) {
        console.log(err.message, err)
        res.send(Res.Fail(err.message || '更新食品信息失败'))
      }
    })
  }
  async deleteFood (req, res, next) {
    ...
    if (!food_id || !Number(food_id)) {
      ...
      return res.send(Res.Fail('food_id参数错误'))
    }
    try {
      ...
      await menu.save()
      await food.remove()
      res.send(Res.Success('删除食品成功'))
    } catch (err) {
      console.log('删除食品失败', err)
      res.send(Res.Fail(err.message || '删除食品失败'))
    }
  }
}
